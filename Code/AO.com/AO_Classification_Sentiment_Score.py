...
def RFclassifier(X_train, X_test, y_train, y_test, X):
    ...
    return probs, cm, score, y_pred, y_pred_all

# Apply the classifier formula to the 2 datasets - single words without and with SMOTE
# Print a roc curve plot and save
RF_words, cm, score, y_pred, y_pred_all = RFclassifier(X_train, X_test, y_train, y_test, X)
...
fullset.to_excel('AO FullSet With Predictions.xlsx', sheet_name='sheet1', index=False)
...
file = open('AOcom_F1Scores_Sentiment_Fullset.txt','w')
file.write('AOcom_F1Scores_Sentiment_Fullset.txt' + str(score_tb_all)+'\n')
