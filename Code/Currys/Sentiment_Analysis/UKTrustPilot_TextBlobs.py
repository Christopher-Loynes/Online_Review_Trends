...
def InitialWriteToFile():
	...
	# To_Write = ["Reviews", "Stars", "Category","Sentiment1", "Sentiment2", "Sentiment3"]
	to_Write = ["Reviews", "CurrysReply","Number of Stars","Location", "Category",
                    "Sentiment1", "Sentiment2", "Sentiment3"]
	fileN = "SentimentAnalyse_TextBlob_UKTrustPilotCurrys_V2.csv"
...
def FinalWrite(Review, CurrysReply, Stars,Location,Category,Sentiment1, Sentiment2, Sentiment3):
	fileN = "SentimentAnalyse_TextBlob_UKTrustPilotCurrys_V2.csv"
	to_Write = [str(Review),str(CurrysReply), str(Stars), str(Location),str(Category),
                    str(Sentiment1), str(Sentiment2), str(Sentiment3)]
	with open(fileN, 'a') as data_file:
		csv_writer = csv.writer(data_file, delimiter = ',')
		csv_writer.writerow(to_Write)
		data_file.close()
...
def AnalyseSentiments():
	...
	with open(fileName, 'r') as f:
		...
		for row in reader:
			...
			l += 1
			try:
				if l == 112:	#Break at the last row. Avoids errors
					break
		...
		try:
			for s in res["sentences"]:
				for token in s["tokens"]:
					...
					if str(token["pos"]) == "NN" or str(token["pos"]) == "NNS" or
					str(token["pos"]) == "NNP" or str(token["pos"]) == "NNPS":
						flag = 1
					if str(token["pos"]) == "VB" or str(token["pos"]) == "VBG" or
					str(token["pos"]) == "VBD" or str(token["pos"]) == "VBN" or
					str(token["pos"]) == "VBP" or str(token["pos"]) == "VBZ":
						flag = 1
			if flag == 1:
				pos = []
				flag = 0
				...
				try:
					FinalWrite(str(reviewWithoutStopWords), str(allReviewData_Final[counter][1]),
                                                   ...
                                                   str(allReviewData_Final[counter][6]),str(classification))
					time.sleep(1)
				except(IndexError):
					pass
		except(TypeError):
